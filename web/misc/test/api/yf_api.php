 ...
 */
class yfApi {
	private $YF_PANEL      = "http://127.0.0.1:7200"; // 面板地址，请替换为您真实的地址
	private $YF_APP_ID     = 'exampleAppId';
	private $YF_APP_SERECT = 'exampleAppSecret';

	// 如果希望管理多台面板，可以在实例化对象时，将面板地址与密钥传入
	public function __construct($YF_panel = null, $app_id = null, $app_secret = null) {
		if ($YF_panel) {
			$this->YF_PANEL = $YF_panel;
		}

		if ($app_id) {
			$this->YF_APP_ID = $app_id;
		}

		if ($app_secret) {
			$this->YF_APP_SERECT = $app_secret;
		}
	}
	...
	private function httpPost($url, $data, $timeout = 60) {
		...
		// 设置头部信息
		$headers = [
			'app-id: ' . $this->YF_APP_ID,
			'app-secret: ' . $this->YF_APP_SERECT,
		];
