...
function getLoad(data) {
    ...
    if (Occupy <= 30) {
        LoadColor = '#20a53a';
        AverageText = (window.lan && lan.index && lan.index.load_smooth) || t('index.load_smooth', '运行流畅');
    } else if (Occupy <= 70) {
        LoadColor = '#6ea520';
        AverageText = (window.lan && lan.index && lan.index.load_normal) || t('index.load_normal', '运行正常');
    } else if (Occupy <= 90) {
        LoadColor = '#ff9900';
        AverageText = (window.lan && lan.index && lan.index.load_slow) || t('index.load_slow', '运行缓慢');
    } else {
        LoadColor = '#dd2f00';
        AverageText = (window.lan && lan.index && lan.index.load_block) || t('index.load_block', '运行堵塞');
    }
...
$('#LoadList .mask').on('mouseenter', function() {
    ...
    var load1 = (window.lan && lan.index && lan.index.load_1min) || t('index.load_1min', '最近1分钟平均负载：');
    var load5 = (window.lan && lan.index && lan.index.load_5min) || t('index.load_5min', '最近5分钟平均负载：');
    var load15 = (window.lan && lan.index && lan.index.load_15min) || t('index.load_15min', '最近15分钟平均负载：');
    var text = load1 + one + '</br>' +
               load5 + five + '</br>' +
               load15 + fifteen;
...
function showCpuTips(rdata){
    $('#cpuChart .mask').off().on('mouseenter', function() {
        ...
        var cpuPhysical = (window.lan && lan.index && lan.index.cpu_physical) || t('index.cpu_physical', '个物理CPU，');
        var cpuCore = (window.lan && lan.index && lan.index.cpu_core) || t('index.cpu_core', '个物理核心，');
        var cpuLogical = (window.lan && lan.index && lan.index.cpu_logical) || t('index.cpu_logical', '个逻辑核心</br>');
        layer.tips(rdata.cpu[3] + "</br>" + rdata.cpu[5] + cpuPhysical + (rdata.cpu[4]) + cpuCore + rdata.cpu[1] + cpuLogical + cpuText, this, { time: 0, tips: [1, '#999'] });
    }).on('mouseleave', function() {
        layer.closeAll('tips');
    });
}
...
function clearSystem() {
    var cleanMsg = (window.lan && lan.index && lan.index.cleaning_up_system_junk) || t('index.cleaning_up_system_junk', '正在清理系统垃圾');
    var loadT = layer.msg(cleanMsg + ' <img src="/static/img/ing.gif">', { icon: 16, time: 0, shade: [0.3, "#000"] });
    $.get('/system?action=ClearSystem', function(rdata) {
        layer.close(loadT);
        var doneMsg1 = (window.lan && lan.index && lan.index.cleanup_complete_total_of) || t('index.cleanup_complete_total_of', '清理完成,共清理[');
        var doneMsg2 = (window.lan && lan.index && lan.index.files_release) || t('index.files_release', ']个文件,释放[');
        var doneMsg3 = (window.lan && lan.index && lan.index.disk_space) || t('index.disk_space', ']磁盘空间!');
        layer.msg(doneMsg1 + rdata[0] + doneMsg2 + toSize(rdata[1]) + doneMsg3, { icon: 1 });
    });
}
...
function setMemImg(info){
    ...
    if (memFree/(1024*1024) < 64) {
        var warnText = (window.lan && lan.index && lan.index.mem_warning) || t('index.mem_warning', '当前可用物理内存小于64M，这可能导致MySQL自动停止，站点502等错误，请尝试释放内存！');
        $("#messageError").show();
        $("#messageError").append('<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;">' + warnText + '</span></p>');
    }
}
...
function getInfo() {
    $.get("/system/system_total", function(info) {
        ...
        // if (info.isuser > 0) {
        //     $("#messageError").show();
        //     $("#messageError").append('<p><span class="glyphicon glyphicon-alert" style="color: #ff4040; margin-right: 10px;"></span>' + (lan && lan.index && lan.index.user_warning) + '<span class="c7 mr5" title="此安全问题不可忽略，请尽快处理" style="cursor:no-drop"> [不可忽略]</span><a class="btlink" href="javascript:setUserName();"> [立即修改]</a></p>')
        // }
        setImg();
    },'json');
}
...
function getGpuInfo() {
    $.get('/system/get_gpu_info', function(res) {
        ...
        if (info) {
            ...
            var modelLabel = (window.lan && lan.index && lan.index.gpu_model) || t('index.gpu_model', '型号: ');
            var tempLabel = (window.lan && lan.index && lan.index.gpu_temp) || t('index.gpu_temp', '显卡温度: ');
            var memUsageLabel = (window.lan && lan.index && lan.index.gpu_mem_usage) || t('index.gpu_mem_usage', '显存使用率: ');
            var tips = modelLabel + info.name + "<br>" + tempLabel + (info.temperature || '-') + "°C<br>" + memUsageLabel + memUse + "%";
...
setTimeout(function() {
    $.get('/system/update_server?type=check', function(rdata) {
        ...
        if (rdata.data != undefined) {
            var updateText = (window.lan && lan.index && lan.index.update) || t('index.update', '更新');
            $("#toUpdate").html('<a class="btlink" href="javascript:updateMsg();">' + updateText + '</a>');
            $('#toUpdate a').html(updateText + '<i style="display: inline-block; color: red; font-size: 40px;position: absolute;top: -35px; font-style: normal; right: -8px;">.</i>');
...
function showAllLoginLogs() {
    var modalHtml = '<div class="pd15 all-login-logs-modal" style="padding: 15px 20px;">\
        <div class="tootls_group" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px;">\
            <div style="display: flex; align-items: center; gap: 8px;">\
                ...
                <select id="loginLogStatusFilter" class="bt-input-text" style="height: 28px; font-size: 12px; padding: 2px 8px; border-radius: 3px;">\
                    <option value="all">' + t('index.all_status', '全部状态') + '</option>\
                    <option value="success">' + t('index.success', '成功') + '</option>\
                    <option value="fail">' + t('index.fail', '失败') + '</option>\
                </select>\
                <span class="f12 c6" style="margin-left: 8px;">' + t('index.method_filter', '方式筛选:') + '</span>\
                <select id="loginLogMethodFilter" class="bt-input-text" style="height: 28px; font-size: 12px; padding: 2px 8px; border-radius: 3px;">\
                    <option value="all">' + t('index.all_methods', '全部方式') + '</option>\
                    <option value="web">Web</option>\
                    <option value="ssh">SSH</option>\
                </select>\
            </div>\
            <div>\
                <button type="button" class="btn btn-default btn-sm" onclick="getAllLoginLogs(1);" title="' + t('logs.refresh_logs', '刷新日志') + '">\
                    <span class="glyphicon glyphicon-refresh" style="margin-right: 4px; font-size: 11px;"></span>' + t('public.refresh', '刷新') + '\
                </button>\
            </div>\
        </div>\
        <div class="divtable" style="max-height: 380px; min-height: 330px; overflow-y: auto;">\
            <table class="table table-hover" id="allLoginLogsTable" style="margin-bottom: 0;">\
                <thead>\
                    <tr>\
                        <th style="width: 65px;">' + t('public.status', '状态') + '</th>\
                        <th style="width: 65px;">' + t('index.method', '方式') + '</th>\
                        <th style="width: 140px;">' + t('index.login_ip', '登录IP') + '</th>\
                        <th style="width: 120px;">' + t('index.location', '归属地') + '</th>\
                        <th>' + t('logs.details', '详情') + '</th>\
                        <th style="text-align: right; width: 140px;">' + t('index.login_time', '登录时间') + '</th>\
                    </tr>\
                </thead>\
                <tbody id="allLoginLogsList">\
                    <tr><td colspan="6" class="text-center c9" style="padding: 35px 0;"><img src="/static/img/ings.gif" style="margin-right: 4px;"> ' + t('index.loading_logs', '正在获取日志...') + '</td></tr>\
                </tbody>\
            </table>\
        </div>\
    ...
    layer.open({
        type: 1,
        title: t('index.login_audit_title', '登录记录审计'),
        area: ['780px', '530px'],
        closeBtn: 1,
        shadeClose: false,
        content: modalHtml,
...
function getAllLoginLogs(page) {
    ...
    $('#allLoginLogsList').html('<tr><td colspan="6" class="text-center c9" style="padding: 30px 0;"><img src="/static/img/ings.gif" style="margin-right: 4px;"> ' + t('public.loading', '正在加载...') + '</td></tr>');
    ...
    $.post('/get_recent_logins', {
        ...
        if (!rdata || !rdata.status) {
            $('#allLoginLogsList').html('<tr><td colspan="6" class="text-center c9" style="padding: 25px 0;">' + t('index.no_login_logs', '暂无登录记录') + '</td></tr>');
        ...
        var list = data.list || [];
        if (list.length === 0) {
            $('#allLoginLogsList').html('<tr><td colspan="6" class="text-center c9" style="padding: 25px 0;">' + t('index.no_matching_logs', '未找到符合条件的登录记录') + '</td></tr>');
            $('#allLoginLogsPage').html('');
            return;
        }
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (item.is_local) {
                ...
                if (cachedLoc) {
                    ...
                    locationHtml = '<span class="f12 c9 ip-loc-pending" data-ip-loc="' + item.ip + '">' + t('index.querying', '查询中...') + '</span>';
                    ipsToQuery.push(item.ip);
                }
            }
        ...
        $('#allLoginLogsList').html('<tr><td colspan="6" class="text-center c9" style="padding: 25px 0;">' + t('public.fetch_fail_retry', '获取失败，请稍后重试') + '</td></tr>');
        $('#allLoginLogsPage').html('');
    });
}