...
function doSetGpuDetect(isEnable) {
    ...
    $.post('/setting/set_gpu_detect', {}, function (rdata) {
        ...
        if (rdata.status && isEnable) {
            layer.confirm('成功开启GPU首页检测！<br><br><span style="color:red">提示：由于浏览器缓存，若首页未显示GPU状态，<br>需返回首页按 <b>Ctrl+F5</b>（Mac按 <b>Cmd+Shift+R</b>）强制刷新才能展示GPU状态。</span>', {
                title: '设置成功',
                icon: 1,
                btn: ['强制刷新，返回首页', '留在当前页']
            }, function() {
...
function setBasicAuthTip(callback){
	var tip = layer.open({
		...
		<ul class="help-info-text c7 explain-describe-list pd15">\
			...
			<li>如忘记密码，可在SSH通过bs命令来关闭BasicAuth验证</li>\
		</ul>\
	...
	if (!basic_auth){
		setBasicAuthTip(function(){
			...
			<ul class="help-info-text c7">\
				...
				<li>开启后，以任何方式访问面板，将先要求输入BasicAuth用户名和密码</li>\
				<li>开启后，能有效防止面板被扫描发现，但并不能代替面板本身的帐号密码</li>\
				<li>请牢记BasicAuth密码，一但忘记将无法访问面板</li><li>如忘记密码，可在SSH通过bs命令来关闭BasicAuth验证</li>\
			</ul>\
		</div>',
				success:function(){
