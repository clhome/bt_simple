...
class sites(object):
    ...
    def __init__(self):
        # nginx conf
        self.setupPath = yf.getServerDir() + '/web_conf'
        self.logsPath = yf.getLogsDir()
...
<body>
    ...
    def stop(self, site_id):
        ...
        if not os.path.exists(path):
            ...
            yf.writeFile(path + '/index.html', default_text)
    ...
    def nginxAddDomain(self, site_name, domain, port):
        ...
        domains = tmp.split(' ')
        if not yf.inArray(domains, domain):
    ...
    def addDomain(self, site_id, site_name, domain):
        ...
        for d in domains:
            ...
            msg = yf.getInfo('网站[{1}]添加域名[{2}]成功!', (site_name, name))
            yf.writeLog('网站管理', msg)
    ...
    def delDomain(self, site_id, site_name, domain, port):
        ...
        if conf:
            ...
            rep = r"listen\s+([0-9]+);"
            tmp = re.findall(rep, conf)
            port_nums = yf.M('domain').where('pid=? AND port=?', (site_id, port)).count()
            if yf.inArray(tmp, port) == True and port_nums < 2:
                rep = r"\n*\s+listen\s+" + port + ";"
    ...
    def deleteALlLogs(self, webname):
        ...
        errLogPath = yf.getLogsDir() + '/' + webname + '.error.log'
    ...
    def delete(self, site_id, path):
        ...
        for x in binding_list:
            tag = yf.getLogsDir() + "/" + webname + "_" + x['domain']
            site_log = tag + ".log"
            site_error = tag + ".error.log"

            if os.path.exists(site_log):
                yf.removeDir(site_log)
    ...
    def nginxAddConf(self):
        ...
        content = yf.readFile(source_tpl)
...
# '''
    ...
    def setSsl(self, site_name, key, csr):
        ...
        yf.writeFile(keypath, key)
        yf.writeFile(csrpath, csr)
...
"""
            if not yf.isSupportHttp3(version):
                http3Header = '';
    ...
    error_page 497  https://$host$request_uri;""" % (certPath, keyPath, http3Header)
        ...
        tmp = re.findall(rep, conf)
        if not yf.inArray(tmp, '443'):
            listen = re.search(rep, conf).group()

            quic_conf = "quic reuseport"
            if yf.isVhostHasReuseport():
                quic_conf = "quic"

            if yf.isSupportHttp3(version):
                http_ssl = "\n\tlisten 443 ssl;"
                http_ssl = http_ssl + "\n\tlisten [::]:443 ssl;"
    ...
    def getDefaultSite(self):
        data = {}
        data['sites'] = yf.M('sites').field('name').order('id desc').select()
    ...
    def getNgxRewriteDir(self):
        return yf.getPanelDir() + '/web/misc/nginx/rewrite'

    def getNgxTplDir(self):
        return yf.getPanelDir() + '/web/misc/nginx/tpl'
    ...
    def addDirBind(self, site_id, domain, dir_name):
        ...
        if conf:
            ...
            content = yf.readFile(source_dirbind_tpl)
            ...
            content = content.replace('{$OR_REWRITE}', self.rewritePath)
            content = content.replace('{$PHP_DIR}', self.setupPath + '/php')
            content = content.replace('{$LOGPATH}', yf.getLogsDir())

            conf += "\r\n" + content
            yf.backFile(filename)
    ...
    def getDirBindingRewrite(self, binding_id, add):
        ...
        if os.path.exists(filename):
            data['status'] = True
            data['data'] = yf.readFile(filename)
            data['rlist'] = []
    ...
    def delDirBinding(self, binding_id):
        ...
        msg = yf.getInfo('删除网站[{1}]子目录[{2}]绑定',(info['name'], binding_info['path']))
        yf.writeLog('网站管理', msg)
        yf.restartWeb()
    ...
    def setSitePath(self, site_id, path):
        ...
        if content:
            ...
            yf.writeFile(host_conf, content)
    ...
    def setHasPwd(self, site_id, username, password):
        ...
        passconf = username + ':' + yf.hasPwd(password)

        configFile = self.getHostConf(siteName)
        # 处理Nginx配置
        conf = yf.readFile(configFile)
        if conf:
    ...
    #AUTH_END''' % (filename,)
            ...
            yf.writeFile(configFile, conf)
        # 写密码配置
        passDir = self.passPath
        if not os.path.exists(passDir):
            yf.makeDirs(passDir)
        yf.writeFile(filename, passconf)
    ...
    def setSecurity(self, site_id, fix, domains, status, none=''):
        ...
        if os.path.exists(file):
            ...
            if status == 'false':
                ...
                conf = re.sub(rep, '', conf)
                yf.writeLog('网站管理', '站点[' + name + ']已关闭防盗链设置!')
            else:
                rep = r"\s{0,4}#SECURITY-START(\n|.){1,500}#SECURITY-END\n?"
                conf = re.sub(rep, '', conf)
    ...
    def getRedirect(self, site_name):
        ...
        if not os.path.exists(redirect_file):
            ...
            return yf.returnData(True, "no exists!", {"result": [], "count": 0})

        content = yf.readFile(redirect_file)
    ...
    def operateRedirectConf(self, siteName, method='start'):
        ...
        content = yf.readFile(vhost_file)
    ...
    def setRedirect(self, site_name, site_from, to, type, r_type, keep_path):
        ...
        _id = yf.md5("{}+{}".format(file_content, site_name))
        ...
        yf.writeFile(redirect_file, json.dumps(data))
        yf.writeFile("{}/{}.conf".format(self.getRedirectPath(site_name), _id), file_content)
    ...
    def delRedirect(self,siteName, rid):
        ...
        try:
            ...
            data_content = yf.readFile(data_path) if os.path.exists(data_path) else ""
            ...
            for item in data:
                if item["id"] == rid:
                    ...
                    break
            # write database
            yf.writeFile(data_path, json.dumps(data))
    ...
    def getProxyList(self, site_name):
        ...
        if not os.path.exists(data_path):
            ...
            return yf.returnData(True, "", {"result": [], "count": 0})

        content = yf.readFile(data_path)
        data = json.loads(content)
        tmp = []
        for proxy in data:
...
"
    ...
    def closeProxyAll(self, site_name):
        ...
        if os.path.exists(proxy_path):
            content = yf.readFile(proxy_path)
            data = json.loads(content)
            for proxy in data:
                proxy_dir = "{}/{}".format(self.proxyPath, site_name)
    ...
    def closeRedirectAll(self, site_name):
        ...
        if os.path.exists(redirect_path):
            content = yf.readFile(redirect_path)
            data = json.loads(content)
    ...
    def saveProxyConf(self, site_name, proxy_id, config):
        ...
        yf.writeFile(proxy_file, config)
        rule_test = yf.checkWebConfig()
        if rule_test != True:
    ...
    def getSsl(self, site_name, ssl_type):
        ...
        if ssl_type == 'lets':
            ...
            # acme_dir = yf.getAcmeDomainDir(site_name)
        ...
        if os.path.exists(key_path):
            key = yf.readFile(key_path)

        csr = ''
        if os.path.exists(csr_path):
            csr = yf.readFile(csr_path)

        cert_data = yf.getCertName(csr_path)
    ...
    def setCliPhpVersion(self, version):
        ...
        yf.execShell("ln -sf %s %s" % (php_pecl_src, php_pecl))
        yf.execShell("ln -sf %s %s" % (php_pear_src, php_pear))
        if is_chattr != -1:
            yf.execShell('chattr +i /usr/bin')
    ...
    def writeAcmeLog(self,msg):
        ...
        yf.writeFile(log_file, msg+"\n", 'w+')
        return True

    def letLogFile(self):
        return yf.getPanelDir() + '/logs/letsencrypt.log'
    ...
    def createAcmeFile(self, site_name, apply_ca, domains, email, force, renew):
        ...
        if not result['status']:
            return result
        result['csr'] = yf.readFile(src_cert)
...
''' % (acme_dir,)
    ...
    def createAcmeDns(self, site_name, apply_ca, domains, email, dnspai, wildcard_domain, force, renew, dns_alias):
        ...
        for k in dnsapi_data:
            if dnsapi_data[k] == '':
                return yf.returnData(False, k+'为空!')
...
''' % (acme_dir,)
    ...
    def createLet(self, site_name, domains, force, renew, apply_type, dnspai, email, wildcard_domain):
        ...
        if os.path.exists(host_conf_file):
            ...
            # 检测存在反向代理
            data_path = self.getProxyDataPath(site_name)
            data_content = yf.readFile(data_path)
            ...
            yf.writeFile(host_conf_file, siteConf)
            yf.restartWeb()
        ...
        src_letpath = yf.getServerDir() + '/web_conf/letsencrypt/' + site_name
        ...
        if not os.path.exists(src_letpath):
            ...
            if not data['status']:
                ...
                return yf.returnData(data['status'], msg, data['msg'])
        ...
        result['csr'] = yf.readFile(src_csrpath)
        result['key'] = yf.readFile(src_keypath)
    ...
    def getBackup(self,site_id,page=1,size=10):
        ...
        data['data'] = info['list']
        data['site'] = site_info
        data['page'] = yf.getPage({'count':info['count'],'tojs':'getBackup','p':page, 'row':size})
        return data
    ...
    def getPhpVersion(self):
        ...
        for val in phpVersions:
            ...
            # 标准判断
            checkPath = yf.getServerDir() + '/php/' + val + '/bin/php'
        ...
        # 其他PHP安装类型
        conf_dir = yf.getServerDir() + "/web_conf/php/conf"
