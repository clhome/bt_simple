...
- [/] jQuery 升级 - 解决遗留的 JQMIGRATE 警告 (阶段 3 补充)
  - [/] 将所有业务代码中的 `.hover()` 替换为 `.on('mouseenter', ...).on('mouseleave', ...)` 兼容写法
  - [/] 清理 `jquery.dragsort-0.5.2.min.js` 中残留的 `.unbind()` / `.bind()` / `.mousedown()` 废弃 API
  - [/] 优化 `scripts/sync_to_test.py` 打包逻辑，确保 `jquery.dragsort-0.5.2.min.js` 和其他修改过的静态库资源在同步时不会被遗漏
  - [ ] 部署到测试服务器，清空缓存并进行回归测试，确保警告降为 0
