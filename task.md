...
- [x] 备份与预备：物理备份待修改的 OpenResty 插件核心文件 @done(2026-05-29 16:15)
- [x] 强杀残留进程，解决端口占用：在 `plugins/openresty/install.sh` 与 `versions/1.29.2/install.sh` 编译安装前，增加强制停止旧服务及强杀所有 `nginx`/`openresty` 进程的逻辑，彻底释放 80 等端口 @done(2026-05-29 16:16)
- [x] 引入配置语法自愈，秒级修复损坏：在 `plugins/openresty/index.py` 的启动、重载与操作逻辑中引入 `confSelfHeal()` 机制，采用高鲁棒的单行正则匹配，将损坏的 `cri` 选项智能、安全地自愈修正为标准的 `crit;` @done(2026-05-29 16:16)
- [x] 验证：确保重装 OpenResty 安装流程完美连通，启动成功率 100%，损坏配置成功自动修复，负载状态页面完美显示 @done(2026-05-29 16:17)
