...
- [x] 用户进行浏览器强制刷新验证（Ctrl + F5），检查调整后的日志展示高度
...
- [x] 验证：确认主界面文件夹大小均显示为“计算”，点击后显示 loading，稍候被真实文件夹大小替代（如 `3.12GB` / `2.15MB`），且不触发整行选中
...
- [x] 验证：在面板“软件列表”中再次点击 `swap` 插件的“更新”按钮，验证其可以顺利将版本号写入 `/www/server/swap/version.pl` 变成 `1.5`，并且不再提示需要“更新”。同时确认原有 `swapfile` 不受任何影响。
...
- [x] 验证：确保重构后代码语法完全正确，各项参数过滤机制完美拦截危险输入，站点高可用及健康检查配置在面板中被正确渲染，无任何运行时崩溃
...
- [x] 备份与预备：物理备份待修改的 `webstats` 插件核心文件
- [x] 升级 `plugins/webstats/info.json` 插件配置，版本更新至 0.2.6，日期更新至 2026-05-29
- [x] 优化 `install.sh` 鲁棒性：引入 `GEO_VERSION` 备用 fallback 默认静态版本，防止网络波动或 API 限频导致安装崩溃
- [x] 彻底修复 `plugins/webstats/index.py` 安全 SQL 注入漏洞，对模糊搜索 `search_uri` 实施严谨的参数化绑定查询
- [x] 彻底重构 `plugins/webstats/tool_migrate.py` 以进行可用性与性能优化：
  - [x] 彻底移除每次迁移站点结束时的 Nginx `restart` 重启操作，保证零服务中断
  - [x] 引入 SQLite 显式大事务机制 `BEGIN TRANSACTION` / `COMMIT`，并改为参数化批量插入，将数据迁移吞吐提升 100x+，杜绝 WAL 锁定和数据库损坏隐患
- [x] 安全彻底清理并物理删除未被引用的冗余文件 `plugins/webstats/webstats_index.py`
- [x] 验证：确保重构后代码语法完全正确，迁移任务运行飞速，无任何 Nginx 中断，且安全性注入漏洞被成功封锁
...
- [x] 重构 Dockerfile，使用 debian:12-slim，拆分构建层，增加缓存清理
- [x] 移除 start.service 和旧的 start.sh
- [x] 新增 entrypoint.sh，生成动态密码并守护进程
- [x] 修复 VOLUME，仅映射 /www/wwwroot 和 /www/server/mysql/data
...
## 修复 pgadmin 插件 503 错误

**需求**
pgadmin 插件在启动时报 503 错误，原因是 `pg_init.sh` 中使用 `expect` 脚本自动填充交互式输入，但在最新版 pgadmin4 且部分系统无 expect 依赖时会失败。需要重构为环境变量方式的静默安装，并确保数据库目录提前存在。

### Task List

- [x] 修改 `plugins/pgadmin/pg_init.sh`，使用 `PGADMIN_SETUP_EMAIL` 替代 `expect`，并执行 `setup-db`。
- [x] 在 `pg_init.sh` 中增加创建 `data/pgadmin4` 目录逻辑。

## 需求：phpmyadmin插件服务页面增加访问信息

**问题描述：**
在 phpMyAdmin 插件的服务页面下方，增加内网访问地址、外网访问地址、用户名、密码以及 root 密码的获取提示。要求样式优雅现代。
...
- [x] 在 plugins/phpmyadmin/index.py 中增加 get_pma_access_info 接口，返回内网地址、外网地址、用户名和密码
- [x] 在 plugins/phpmyadmin/index.html 中调用自定义服务渲染函数
- [x] 在 plugins/phpmyadmin/js/phpmyadmin.js 中编写 pmaService() 函数，在渲染原始服务信息后，追加访问信息的现代化卡片面板
...
- [x] 修改 web/static/app/index.js，将 $("#state").html(info.cpuRealUsed) 格式化为 parseFloat(info.cpuRealUsed).toFixed(1)
- [x] 修改 web/static/app/index.js 中 getNet 函数的 $("#state").html(net.cpu[0])，同样格式化为1位小数
...
- [x] 编写前端 plugins/jdk/index.html，实现 JDK 列表和操作按钮（安装、设为默认、卸载）。
...
1. **动态调整 PHP-FPM 进程数防卡死** (`plugins/php-apt/index.py`): 在 `phpFpmWwwReplace` 环节引入了基于系统总内存的梯队计算逻辑。现在无论是 1G 小鸡还是 8G 以上的高配机，安装或重启 PHP 时都能自动分配合适的 `max_children`（并发进程），避免小内存 OOM，或大内存下只有 2 个进程处于“闲置摸鱼”状态导致请求卡死。
2. **自动化预设高性能、安全的 php.ini** (`plugins/php-apt/index.py`): 在 `initReplace` 环节中，除了原有的 SSL 设置，额外增加了十几个核心性能指标和安全函数的修改项。这包括了：
   - 自动设置 `post_max_size` 与 `upload_max_filesize` 为 `50M`
   - 时区强制设置为 `PRC`
   - 开启 `short_open_tag` 兼容性
   - 封禁了诸如 `exec`, `system`, `passthru`, `shell_exec` 等40多个高危后门函数（直接大幅拔高安全性）
...
## 需求：优化 Pure-FTPd 安装脚本安全性与鲁棒性

**问题描述：**
当前的 `pureftp` 插件在安装过程中存在两点不足：
1. `--with-tls` 虽然编译加入了，但未默认生成证书并开启，FTP 数据传输仍是明文的。
2. 安装成功与否的检测只判断了目录存在，若 `make` 阶段出错仍可能被判定为安装成功。
...
- [x] 强化 `plugins/pureftp/install.sh` 安装成功检测：必须检测核心文件 `bin/pure-pw` 是否成功编译，如果失败直接异常退出并清理目录。
- [x] 增强 `plugins/pureftp/install.sh` TLS 支持：安装完成后自动生成 TLS DH Param 及证书文件，并更新配置文件 `pure-ftpd.conf`，随后重启服务。

## 分析与记录：MongoDB 安装脚本优化借鉴点

**分析背景：**
比对参考的优秀脚本 `参考/mongodb.sh` 与本面板目前的 `plugins/mongodb/install.sh` 及各个子版本脚本（如 `versions/7.0/centos.sh`），发现参考脚本在架构设计和容错处理上有许多值得借鉴之处。

**参考脚本值得借鉴的核心优势：**

1. **统一入口与动态判定（避免碎片化）**：
   - 参考脚本只有一个 `mongodb.sh`，在最前端统一处理 `OS_SYS`、`OS_VER` 的判定，将所有不同系统环境的下载逻辑归一化处理。
   - 当前面板将不同大版本与不同系统拆分成了数十个 `.sh` 文件（如 `versions/7.0/centos.sh`、`versions/7.0/ubuntu.sh` 等），导致下载路径和解压逻辑极度冗余，后期更新维护成本高。
2. **底层硬件指令集强校验（AVX）**：
   - MongoDB 5.0+ 强依赖 CPU 的 AVX 指令集。参考脚本特意通过 `check_avx` 函数对 `/proc/cpuinfo` 进行了拦截，不支持的 CPU 拒绝安装，避免用户安装完成启动后发生 Core Dump 才报错。当前插件缺乏该硬件校验机制。
3. **更精细的 OS 与 MongoDB 版本匹配保护**：
   - 参考脚本中对例如 `Ubuntu-24` 拒绝安装 `4.4`，对 `Debian-12` 拒绝安装 `6.0` 做了清晰的前置判断。防止用户下载并安装了不兼容的版本。
4. **精细化的权限管控**：
   - 自动检测并建立 `mongo` 用户组与无 Shell 环境登录的系统用户，而当前面板脚本部分注释掉了该环节。
5. **对软链接的环境变量覆盖**：
   - 强制将 `/bin/` 里的可执行文件 `ln -sf` 软链接到了 `/usr/bin/` 下，方便用户终端直调。当前面板使用 python `index.py` 控制为主，缺少这部分提升用户体验的操作。

**建议后续优化任务清单：**

- [x] 重构 MongoDB 安装逻辑：废弃按 OS 拆分的碎片化脚本，重构成统一按系统参数拉取文件的核心安装脚本。 @done(2026-06-15 09:18)
- [x] 增加 AVX 指令集拦截：在安装 5.0 及其以上的 MongoDB 版本前，通过 `grep -o avx /proc/cpuinfo` 强制拦截不支持的旧 CPU。 @done(2026-06-15 09:18)
- [x] 增加系统版块白名单控制：对特定系统环境提示最新兼容版本，规避下载后发现不能用的问题。 @done(2026-06-15 09:18)
- [x] 优化软链接与用户组：恢复对 `mongo` 用户的隔离绑定以及向 `/usr/bin/` 下发可执行命令等体验细节。 @done(2026-06-15 09:18)

## 分析与架构设计：Fail2Ban 插件深度重构与 OP_WAF 联动机制

**分析背景：**
比对参考的优秀脚本 `参考/fail2ban/fail2ban_main.py` 与本面板目前的 `plugins/fail2ban/index.py`，发现参考版本是一个高度自动化的 Fail2Ban 规则生成和策略管理中心，具有极高的借鉴价值。同时，为了避免与现有的 `op_waf` (Web应用防火墙) 发生功能冲突，设计了两者协同工作的最佳实践模式。

**参考代码核心借鉴点：**
1. **服务规则自动化生成**：针对 SSH、FTP、Dovecot、Postfix、MySQL 等非 Web 服务的防爆破配置提供了一键自动化生成支持，并能自动定位这些服务的默认日志路径。
2. **网站级别的精准防护**：支持识别 Nginx、Apache 等多 Web Server，针对单一站点自动提取 `access.log` 路径进行 CC/扫描 防护。
3. **高级日志时间戳智能识别**：内置了 7 种不同时间戳格式的自动探测（如 ISO 8601、Apache 格式、Syslog），极大降低了自定义规则对 `datepattern` 的配置门槛。
4. **动态生成正则表达式**：通过一段“样本日志”和“关键字”，能自动推导与泛化成 `failregex`，简化防爆破策略书写难度。
5. **规范化的全局 IP 白名单**：支持多种格式的 IPv4/IPv6 全局 `ignoreip` 写入与管理。

**Fail2Ban 与 OP_WAF 的联动机制设计（七层与四层结合）：**
* **重叠性分析**：`op_waf` 工作在第七层（Nginx内部），实时消耗资源进行请求拦截；`fail2ban` 工作在第四层（iptables/firewalld），后置处理但彻底断开连接。二者在 Web 防护目标上有重叠。
* **最佳实践架构（联动模式）**：
  不让 Fail2Ban 直接去分析 Nginx 的普通 `access.log`，而是配置 Fail2Ban 去监听 `op_waf` 产生的 **拦截告警日志**。
* **优雅的机制（带回退方案）**：
  - **同时安装了 OP_WAF 与 Fail2Ban（联动模式）**：WAF 承担细粒度的七层过滤并输出拦截日志；Fail2Ban 监控该日志，一旦发现某 IP 被 WAF 拦截超过设定阈值（如 5分钟内50次），则 Fail2Ban 直接在 iptables 层封杀该 IP。极大减轻 Nginx 的 CPU 与连接压力。
  - **仅安装了 Fail2Ban（独立模式/回退方案）**：无 WAF 提供拦截日志，Fail2Ban 降级直接读取网站的原始 `access.log`，并利用上述第 2、3、4 点能力，通过内置规则（正则匹配特定的高频访问或特定路径扫描）来实现防扫描和防 CC。
  - **仅安装了 OP_WAF（独立模式）**：WAF 依靠自带的拦截引擎与 IP 封禁模块（如基于 Nginx 共享内存的拉黑机制）独立工作，虽然消耗少量 Nginx 资源，但仍能完成拦截任务。

### Task List

- [x] 吸收参考版本：移植非 Web 服务（SSH/FTP/MySQL 等）的一键防爆破功能及全局 IP 白名单能力到 `fail2ban/index.py`。
- [x] 吸收参考版本：移植日志时间戳探测器 `_detect_timestamp_pattern` 及动态正则生成器 `_generate_failregex_from_log`。
- [x] 架构实现：在 `fail2ban` 插件中增加“Web防扫描与防CC”配置逻辑。
  - 当检测到已启用 `op_waf` 时，引导配置为“监听 OP_WAF 拦截日志”的高效联动惩罚模式。
  - 当未安装 `op_waf` 时，回退执行直接监控 `access.log` 匹配正则的原始 Fail2Ban CC防护。
### 前端交互重构与 UI 设计 Task List

- [x] 在 `plugins/fail2ban/index.html` 的侧边栏导航中增加“系统防护”、“网站防护”两个新菜单项。
- [x] 在 `plugins/fail2ban/js/fail2ban.js` 中开发“系统防护”界面，支持读取和展示当前启用的服务防护状态（如 SSH、FTP、MySQL），并支持一键开启/修改/关闭防护。
- [x] 在 `plugins/fail2ban/js/fail2ban.js` 中开发“网站防护”界面，支持动态拉取当前所有已建站点列表，并支持一键开启防扫描或防 CC 功能。
- [x] 结合后端的 OP_WAF 联动状态，在“网站防护”界面增加 OP_WAF 联动模式的 UI 视觉提示（如果是联动模式则显示高亮 Badge 提示“高级 WAF 联动惩罚已启用”）。

## 需求：部署脚本添加安装完毕上报请求与提示

**问题描述：**
在安装最后阶段访问地址 `curl --location --request GET 'https://www.yftec.top/index.php/api/deploy/report'`，根据返回的 `user_number` 字段，显示文字“您是御风面板第{user_number}位用户，衢州御风科技与您智领未来，共筑匠心”。
...
- [x] 在 `deploy.sh` 的 `show_panel_info` 末尾，添加 curl 请求与 json 解析逻辑 @done(2026-06-15 14:29)
- [x] 打印包含 `user_number` 的提示语 @done(2026-06-15 14:29)

## 需求：借鉴 bt_install_panel.sh 优化 deploy.sh 增加容错与预检

**问题描述：**
当前的 `deploy.sh` 脚本在系统资源不足或遭遇后台包管理锁时容易中断失败，且未拦截已不支持的旧系统。
需要通过引入系统检查和 APT 解锁等机制，提升脚本安装成功率。
...
- [x] 在 `deploy.sh` 中添加 `check_architecture` 和 `check_system_resources` 以执行内存及磁盘空间检查。
- [x] 添加 `check_os_version` 拦截不支持的老系统版本。
- [x] 添加 `fix_apt_lock` 机制，在 `apt` 操作前自动清理和释放 DPKG/APT 锁。
- [x] 修改 `install_basic_deps` 等方法，在操作前后植入上述预检机制并进行二次安装验证。

## 需求：提升 deploy.sh 安装效率（添加国内镜像加速机制）

**问题描述：**
针对国内网络环境，`deploy.sh` 默认使用官方源安装大量 Python 依赖和系统依赖，耗时极长。
需要添加镜像源自动替换逻辑，实现网络加速。
...
- [x] 在 `deploy.sh` 中编写 `setup_domestic_mirrors` 函数。
- [x] 仅当 `check_china` 为真时触发：注入 `~/.pip/pip.conf` 配置阿里云 Pip 源。
- [x] 仅当 `check_china` 为真且操作系统为 Ubuntu/Debian 时，将官方慢速源批量替换为 `mirrors.aliyun.com`。
- [x] 在 `install_basic_deps` 执行前调用上述加速方法。

## 需求：修复 GitHub 代理单点故障，实现全局 Git 高可用

**问题描述：**
`deploy.sh` 中的 `setup_china_git_config` 存在单点故障风险，硬编码了 `gh-proxy.org` 作为所有后续 `git clone` 的代理。
若该代理下线，将导致所有插件安装失败。需实现节点存活检测和动态择优录取。
...
- [x] 重构 `setup_china_git_config`，引入 `_GH_PROXY_LIST` 中的备用代理列表。
- [x] 添加基于 `curl` 和 `git-upload-pack` 端口的探针进行轻量级连通性测试。
- [x] 动态挑选首个存活的节点注入全局 Git config，并在全挂时优雅回退直连。

## 需求：优化 github_download.sh 摒弃低效轮询，引入探针测速与全局缓存

**问题描述：**
当前的 `github_download.sh` 脚本虽然具有代理 fallback 功能，但由于是带着完整超时设置进行顺序尝试，若遭遇前方代理连续宕机，会导致数分钟的“空转”浪费。
...
- [x] 引入 `_GH_BEST_PROXY` 内存缓存变量机制。
- [x] 编写前置 `_gh_get_best_proxy` 函数，使用极短超时的 `curl` 对列表代理进行一次性极速探测并赋值给缓存。
- [x] 修改底层的 `github_download`, `github_clone` 和 `github_api_get` 逻辑，优先读取缓存高可用节点执行下载。

## 需求：修复 git clone 极端断流场景下的脏目录残留漏洞

**问题描述：**
若 `git clone` 在中途因网络原因被 `timeout` 强杀，可能会残留不完整的 `.git` 文件夹。当前仅靠检测目录存在的机制会误判为成功，引发灾难性后果。
...
- [x] 在 `_gh_try_clone` 中捕获克隆命令的严格退出码 `$?`。
- [x] 增加双重校验逻辑：退出码必须为 0 且目录完整。
- [x] 校验失败后强制清理目标脏目录，以确保退回安全轮询状态。

## 需求：优化 OpenResty 其他版本的安装脚本（引入 Jemalloc 与实用第三方模块）

**问题描述：**
参考 OpenResty 1.27.1 安装脚本，为其他版本的 install.sh 引入 Jemalloc 内存分配器优化，并增加 
gx_cache_purge、
ginx-http-concat 和 
gx_http_substitutions_filter_module 三个常用且强大的第三方模块。目前仅在 1.27.1 中完成，需要同步到其他所有常用版本。
...
- [x] 在上述文件中增加架构检测，针对非 ARM 架构下载并编译 Jemalloc 5.3.0。
- [ ] 在上述文件中下载 
gx_cache_purge、
ginx-http-concat 和 
...
- [x] 修改 public.js 中的 pluginOpService，将原来单次 2 秒的 setTimeout 刷新改为 setInterval 静默轮询（每 2 秒一次，共计 5 次），以解决由于 MySQL 等重型服务启动慢导致面板外部状态无法及时同步变化的问题。
- [x] 修复 index.py 中 getMasterStatus 函数未对 db.query 的异常返回值进行类型检查导致迭代 OperationalError 抛出 TypeError 从而显示长串报错日志的 BUG。
- [x] 修改 install.sh 安装脚本，在 start 和 initd_install 之后追加调用 ixDbAccess 函数，实现 MySQL 极速安装结束后如果遇到旧版残留数据自动执行新随机密码强制覆盖的自愈逻辑。
- [x] 修改 plugins/mysql/conf/ 目录下的所有 MySQL 8.x 和 9.x 配置文件模板，增加 mysqlx = 0 以默认禁用 MySQL Document Store (X Protocol) 服务，避免不必要的 33060 端口监听并节省系统资源。
- [x] 优化 MySQL 和 MariaDB 的安装前置校验逻辑：将原本仅判断 [ -d $serverPath/mysql ] 这种简单的目录存在检查，改为检测内部核心的二进制执行文件（例如 in/mysql）。如果仅发现了空目录或残缺目录（通常是因为中途手动终止任务导致残留），则不再误报“服务已存在”，而是自动清理并继续执行新的安装流程，实现“软回退”和容错处理。
- [x] 修复 getSList 在安装带有后缀版本（如 8.4-fast）时无法正确显示“正在安装”状态的 Bug。由于原本的切分逻辑 	mpt.split('-') 会将含有横杠的版本号误切分，导致 	ask_name 无法匹配上实际的 
ame（如 mysql-8.4 不等于 mysql），从而使得 UI 获取不到任务进度（直接变成安装好的状态）。现已更新 web/utils/plugin.py 中 checkSetupTask 和 
efreshDynamicStatus 任务状态的获取逻辑。
- [x] 修复 mysql 极速安装包下载完毕后显示“文件未生成”的 Bug。原因是在 WSL 或特定 Windows 环境挂载下，大文件直接下载到类似 /www/server/source/mysql 等受 Windows 文件系统（DrvFs）控制的目录时，会在下载完成修改时间戳（utime）或立刻被 Bash 检测时因文件同步延迟/杀毒软件锁定而产生 No such file or directory 错误。现已将下载逻辑调整为先下载至安全的 Linux 内部缓存目录 /tmp 中，校验无误后再原子化移动到目标位置。


## 首页系统详细信息弹窗
- [x] 后端 API: 在 system.py 中新增 /get_system_details 接口
- [x] 后端 API: 完善 main.py 中的系统底层信息获取逻辑
