...
## 插件后端 returnJson 中 k_ 哈希提示统一治理与 6 国语言适配（0 残留、100% 自然语言架构）

- [x] 351. 全量解析并生成 34 个插件 795 处 `k_xxxx` 哈希码与原始自然中文/参数拼接的精准对应映射字典（`test/build_resolved_k_dict.py`，100% 覆盖率验证已就绪）。
- [x] 352. 批量安全还原 34 个插件 `index.py` 中的 795 处 `k_xxxx` 为清晰直观的自然语言中文与正确拼接。
- [x] 353. 修复前端展示层（如 `linux_sys_opt/index.html` 等），确保弹出提示通过 `pt(rdata.msg)` 处理，消除多语言环境下的提示死角。
...
## Linux系统优化插件（linux_sys_opt）右上角内存匹配提示多语言精简与专业缩写优化

- [x] 361. 梳理与精炼各语言包（`en`, `de`, `fr`, `it`, `zh-TW`）中右上角内存匹配提示的 4 个关键字段（“当前内存为：”、“，优化脚本会”、“自动匹配”、“不同内存阶梯配置”），采用专业运维紧凑简写规范。
