...
## 第二十七阶段：pg_docker 插件弹窗滚动条与框体宽度优化 (完成)

- [x] 1. 修复备份管理弹窗（`showBackups`）超出高度缺失滚动条问题：在内容外层容器设置 `height: 100%; box-sizing: border-box; overflow-y: auto;`，并调整弹窗宽度至 820px，确保超长列表能顺畅纵向滚动。
- [x] 2. 修复插件主弹窗宽度不足导致实例列表操作列换行问题：将主弹窗自适应宽度调整至 1100px 并自动居中，为操作列表头及单元格增加 `white-space: nowrap;` 与充足列宽，保证“备份/明细/修改/卸载”单行优雅呈现。
...
## 第三十阶段：优化 op_waf 侧边栏灰色底纹及固定弹窗高度 (完成)

- [x] 1. 在 `plugins/op_waf/index.html` 中引入 `resetPluginWinHeight(620)`，并为 `.bt-w-main`、`.bt-w-con`、`.bt-w-menu` 锁定固定高度与统一纵向滚动，彻底消除切换各菜单时弹窗高度忽高忽矮的抖动问题。
- [x] 2. 在 `plugins/op_waf/index.html` 中优化左侧菜单栏样式：统一设置为浅灰色底纹（`#f8fafc`），未选菜单透明，选中菜单高亮为白色卡片浮起态（`#ffffff` + 绿色高亮文本），并优化“功能介绍”页面的容器高度避免双滚动条。
- [x] 3. 编写与运行自动化测试用例，验证样式规则与高度固定逻辑。

## 第三十一阶段：将弹窗尺寸固定、左侧灰色底纹与单行指标卡片规范写入《插件统一开发规范.md》 (完成)

- [x] 1. 在 `文档/插件统一开发规范.md` 中新增第 5 节《插件弹窗与前端 UI/UX 强制规范》，明确规定：
  - 弹窗必须显式调用 `resetPluginWinHeight` 和 `resetPluginWinWidth` 锁定高度，消除切换 Tab 时的高低抖动；
  - 左侧侧边栏（`.bt-w-menu`）统一使用全高浅灰底纹（`#f8fafc`），未选中透明，选中项必须高亮为白色立体卡片（`#ffffff` + 主题色文字）；
  - 多指标/规则卡片必须采用单行等分 Flex 布局（`flex: 1; min-width: 0;`），严禁非对称折行；
  - 统一由右侧 `.bt-w-con` 进行内部纵向滚动，严禁子页面硬编码局部高度导致双滚动条；
  - 静默后台统计数据必须使用 `api.postSilent()` 并在前端做防御性安全解析。
- [x] 2. 检查并校对文档排版与编号，更新 task.md 标记完成。

## 第三十二阶段：批量统一 37 个正式插件的弹窗尺寸固定与左侧灰色底纹规范 (完成)

- [x] 1. 编写批量统一重构脚本并对 37 个正式插件执行应用：
  - 大型服务/多配置/监控类插件（14个）：应用 1050x620/650 标准尺寸与统一样式；
  - 中型语言/运行时/工具类插件（15个）：应用 850x580/620 标准尺寸与统一样式；
  - 轻量级小工具类插件（5个）：应用 750~800x480~530 标准尺寸与统一样式；
  - 单页型插件（3个）：应用专用尺寸固定与滚动保护。
...
## 第四十阶段：监控页面性能优化、信息整合与多语言适配 (完成)

- [x] 1. 扩充 6 国语言词典（zh-CN, zh-TW, en, fr, de, it）：在 `template.json` 和 `lan.js` 中录入计算资源 (CPU & 内存)、系统负载、全局时间范围、多图表联动等新增监控词条。
- [x] 2. 重构前端模板 `web/templates/default/monitor.html`：优化为 4 卡片 2x2 聚合响应式布局，顶部增加全局时间范围切换控件，卡片标题使用标准国际化。
- [x] 3. 重构前端逻辑 `web/static/app/control.js`：
  - 合并 CPU 与内存请求，单次调用 `/system/get_cpu_io` 渲染计算资源双折线图，彻底消除重复请求；
  - 优化负载、磁盘、网络图表加载逻辑，引入结构化的全局与局部时间联动；
  - 引入 ECharts 实例连接联动（`echarts.connect`），实现跨图表十字光标同步；
  - 保证图表放大弹窗（`enlargeChart`）在聚合图表下正常工作。
- [x] 4. 编写全自动回归测试脚本 `test/test_monitor_optimization.py`：校验模板 Jinja2 渲染、JS 语法、无重复请求分发与 6 国语言词条 100% 对齐。
- [x] 5. 运行全量测试套件进行回归验证，确保 0 报错。

## 第四十一阶段：系统监控顶栏精细对齐、卡片头部精简与全局底部时间滑块重构 (完成)

- [x] 1. 优化顶部框体（`.pos-box`）内所有组件布局：实现文字、按钮、输入框、开关与时间切换组件严格水平居中对齐（垂直中心线对齐），消除高低错位。
- [x] 2. 精简 4 个卡片头部：移除各卡片右上角重复的“今天/昨天/7天/30天/自定义”组件，统一收拢至顶部全局控制栏，每个卡片仅保留放大全屏按钮。
- [x] 3. 移除各图表内置的底部独立滑块（slider dataZoom），释放图表纵向绘图空间（减小 grid.bottom）。
...
## 第四十七阶段：时序数据均值降采样与鼠标静止 100ms 防抖同步触发详情 (完成)
...
- [x] 2. 实现鼠标移动静默与停顿 100ms 防抖同步触发：配置 `tooltip.triggerOn = 'none'`，在鼠标移动过程中完全静默不计算 Tooltip（极大降低系统负载），当鼠标停留在图表上超过 20ms 时，统一由 `convertFromPixel` 锁定目标 `dataIndex` 并一次性派发给全部 4 个图表同步显示详情。
