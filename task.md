...
# Task: 在更新部署面板时自动且安全地更新所有插件

## 项目描述

在更新或迁移面板时，通过自动执行仓库中 `plugins` 目录代码的覆盖同步，解决用户服务器上插件版本陈旧（如 PostgreSQL 管理器未能随之更新等）的问题。为将风险降到最低，自动更新需遵循安全同步原则：只对仓库中已经存在的插件文件夹进行覆盖拷贝（`cp -rf`），而不使用破坏性全删（`rm -rf`），以此完美保留用户在本地存放的自定义插件以及插件内的私钥和特定配置文件。

## 开发规范

- 统一使用 UTF-8 (无 BOM) 格式。
- 遵循原有代码风格。
- 安全至上：严禁直接 `rm -rf` 整个 `plugins` 文件夹，必须采用只对现有存在的插件进行 `cp -rf` 的安全覆盖方式。

## Task List

- [X] 在 `task.md` 中登记此自动更新 Task 与 Task List @done(2026-05-20 17:27)
