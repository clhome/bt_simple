...
- [ ] Phase 2: IPv6 与代理池 CC 防护（IP段统计 /24与/64）
- [ ] Phase 2: 浏览器指纹验证（Canvas/WebGL等替代单JS验证）
- [ ] Phase 3: HMAC-SHA256 签名机制（替代MD5）
- [ ] Phase 3: 管理接口 127.0.0.1 限制（remove_waf_drop_ip等）
- [ ] Phase 3: 上传文件深度检测（扩展名、MIME、文件头检测）
- [ ] Phase 4: 现代攻击规则集（SSRF, Log4j, Spring, Fastjson, ThinkPHP, Redis等）
