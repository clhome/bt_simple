...
- **安全加固**：
  ...
  - **核心底层库安全升级**：编译安装低版本 PHP 时的 OpenSSL 核心依赖由古老存在漏洞的 `1.0.2q` 全面安全升级为大分支终极免费安全版 **`1.0.2u`**，彻底修复了 CPU 100% 拒绝服务 (CVE-2022-0778) 等重大安全缺陷。
  ...
  - **大陆智能中转镜像加速**：所有插件及第三方库依赖编译脚本重构为“全球官方原厂源直连 + 大陆智能中转镜像加速 (mirror.ghproxy.com)”，无感免除手动上传大文件的劳顿，提供全向网络环境下最极致的极速部署体验。
  - 密码存储全面升级为 `bcrypt` 算法。
...
- **高水准高保真视觉与物理微交互升级**：
  - **精英级字体系统**：异步引入 Google Fonts 全球顶尖的 `Outfit` 和 `Inter` 现代化无衬线字体，优雅接管系统英文、数字与字符显示，软化面板背景色彩，实现极佳的视觉匀称度。
  ...
  - **重启倒计时防白页**：重置“重启面板”按钮为 10 秒倒计时展示的友好交互提示框，彻底消除了由于服务尚未拉起而导致的页面空白误解，并为重启/修复操作分配鲜明颜色与 Glyphicons 图标区分。
  - **IP智能公网探测**：管理面板左上角 IP 级联升级。在数据库未配置或显示环回口 IP 时，自动通过 `getLocalIp()` 智能探测并展示真实的服务器公网 IP。
- **软件管理与网站配置深度增强**：
  ...
  - **Web服务器配置消除弃用警告**：移除 web/setting.py 中已废弃的 gunicorn gconfig.ssl_version 选项，消除了面板重启时的 Deprecation 警告，且优化了 Gitea 1.26.1、Alist 3.60.0、MySQL 9.7 等一系列插件版本列表。
