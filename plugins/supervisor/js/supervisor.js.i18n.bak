...
function supList(page, search){
    ...
    api.post('get_sup_list', _data, function(data){
        ...
        con += '<div class="code">\
            <span>' + pt('supervisord 常见进程状态详细如下：') + '</span>\
            <span>' + pt('1：STOPPED：该进程已停止。 2：STOPPING：由于停止请求，该进程正在停止。') + '</span>\
            <span>' + pt('3：RUNNING：该进程正在运行。 4：STARTING：该进程由于启动请求而开始。') + '</span>\
            <span>' + pt('5：FATAL：该进程无法成功启动。') + '</span>\
            <span>' + pt('6：BACKOFF：该进程进入“ 启动”状态，但随后退出的速度太快而无法移至“ 运行”状态。') + '</span>\
        </div>'
		...
		layer.open({
	        ...
	        yes: function(index, layero){
				...
				for(var i in options){
		            ...
		            if(opval[options[i]] == ''){
		                if(options[i] == 'user'){
		                    layer.msg(pt('启动用户不能为空'));
		                    return false;
		                }else if(options[i] == 'priority'){
		                    layer.msg(pt('启动优先级不能为空'));
		                    return false;
		                }else if(options[i] == 'numprocs'){
		                    layer.msg(pt('进程数量不能为空！'));
		                    return false;
		                }
		            }
		        }
...
function supLogs(_name, config_tpl_func, read_config_tpl_func,line){
    ...
    var con = '<div><select id="config_tpl" class="bt-input-text mr20" style="width:30%;margin-bottom: 3px;"><option value="0">' + pt('请选择') + '</option></select>\
    			<button id="sup_clear_log" class="btn btn-success btn-sm clear_logs mr5">' + pt('清理日志') + '</button>\
    			<button id="sup_error_log" class="btn btn-success btn-sm clear_logs mr5">' + pt('查看错误日志') + '</button>\
    		</div>';
