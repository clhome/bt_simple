...
def status(version=''):
    if not yf.isAppleSystem():
        ...
        # 2. 备用检测：解析 postmaster.pid 文件获取真实 PID 并验证 /proc 状态
...
def getDbBackupList():
    ...
    r = getDbBackupListFunc(dbname, is_sync)
    bkDir = getBackupDir()
    rr = []
...
def getDbList():
    ...
    clist = conn.where(condition, params).field(
    ...
    count = conn.where(condition, params).count()
    _page = {}
    _page['count'] = count
    _page['p'] = page
    _page['row'] = page_size
...
def doFullSync(version=''):
    ...
    cmd_tmp = getPluginDir() + "/cmd_run.sh"
