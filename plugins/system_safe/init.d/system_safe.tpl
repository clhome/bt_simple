...
sys_stop()
{
    ...
    # 兜底精准清理其他残留 bg_start 进程
    pids=$(ps aux |grep "system_safe/system_safe.py bg_start"|grep -v grep|awk '{print $2}'|xargs)
    for p in $pids
    do
        kill -9 $p &>/dev/null
    done
...
sys_status()
{
    ...
    if [ -f $pid_file ]; then
        ...
        if [ "$pid" != "" ] && ps -p $pid &>/dev/null; then
            echo -e "\033[32msystem_safe service (pid $pid) is running\033[0m"
            return
        fi
    fi
    # 兜底检测
    isStart=$(ps aux |grep "system_safe/system_safe.py bg_start"|grep -v grep|awk '{print $2}'|xargs)
    if [ "$isStart" != "" ]; then
