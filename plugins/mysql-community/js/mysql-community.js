...
function myPost(method,args,callback, title){
    ...
    $.post('/plugins/run', {name:'mysql-community', func:method, args:_args}, function(data) {
        layer.close(loadT);
        if (!data.status){
...
function myAsyncPost(method,args){
    ...
    return syncPost('/plugins/run', {name:'mysql-community', func:method, args:_args});
}
...
function myPostCallbak(method, version, args,callback){
    ...
    var req_data = {};
    req_data['name'] = 'mysql-community';
    req_data['func'] = method;
    req_data['script']='index_mysql_community';
