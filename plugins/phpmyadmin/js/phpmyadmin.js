    ...
    api.post('get_pma_option', {}, function(rdata){
        ...
        var con = '<div class="ver line">\
                ...
                <div class="ver line">\
                    ...
                    <button class="btn btn-success btn-sm" onclick="setPmaChoose()">' + pt('保存') + '</button>\
                </div>\
                <div class="ver line">\
...
function pmaService() {
    $.post('/plugins/run', { name: 'phpmyadmin', func: 'status', version: '', args: '' }, function(statusData) {
        ...
        var opt = '';
        if (status == 'start') {
            opt = '<button class="btn btn-default btn-sm" onclick="pmaOpService(\'stop\')">' + pt('停止') + '</button>' +
                  '<button class="btn btn-default btn-sm" onclick="pmaOpService(\'restart\')">' + pt('重启') + '</button>' +
                  '<button class="btn btn-default btn-sm" onclick="pmaOpService(\'reload\')">' + pt('重载配置') + '</button>';
        } else {
            opt = '<button class="btn btn-default btn-sm" onclick="pmaOpService(\'start\')">' + pt('启动') + '</button>';
        }
        ...
        api.post('get_pma_access_info', '', function(rdata) {
            ...
            '<div class="pma-access-info" style="margin-top: 15px;">' +
                ...
                '<div class="pma-info-footer">' +
                    ...
                    pt('注意：打开网页后输入用户名为 root，密码请前往 MySQL => 管理列表 => root密码 查看') +
                '</div>' +
            '</div>' +
