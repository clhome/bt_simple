...
if [ "$OSID" == "debian" ] && [ "$OSVER" == "13" ];then
    echo "PHP 5.4 不支持在 Debian 13 (trixie) 上安装（依赖的老版本 OpenSSL/工具链已不可用），请选择 PHP 7.2+"
    exit 1
fi
...
if [ ! -f "$sourcePath/php/php${PHP_VER}/main/php_version.h" ];then
	...
	if [ ! -f "$sourcePath/php/php${PHP_VER}/main/php_version.h" ] && [ -f "$sourcePath/php/php-${version}.tar.xz" ]; then
	...
	if [ ! -f "$sourcePath/php/php${PHP_VER}/main/php_version.h" ]; then
		...
		wget -nv --no-check-certificate -O $sourcePath/php/php-${version}.tar.gz https://museum.php.net/php5/php-${version}.tar.gz 2>/dev/null
		if [ -f "$sourcePath/php/php-${version}.tar.gz" ]; then
			tar -zxf $sourcePath/php/php-${version}.tar.gz -C $sourcePath/php/php${PHP_VER} --strip-components=1 2>/dev/null
		fi
	fi
