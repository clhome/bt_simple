            ...
            yes:function(index){
                ...
                if(conn_type == 'key'){
                    ...
                    if (!args['sname'] || !args['password'] || !args['port']){
                        layer.msg(pt('请输入帐号、密码、端口信息'));
                        return false;
                    }
                }

                if ($('input[name="ip"]').val() == ''){
                    layer.msg(pt('请输入服务器IP地址！'));
                    return false;
                }
        ...
        content:'<div class="lsyncd_exclude">\
                ...
                <div>\
                    <ul class="help-info-text c7" style="list-style-type:decimal;">\
                        <li>' + pt('排除的文件和目录是指当前目录下不需要同步的目录或者文件') + '</li>\
                        <li>' + pt('如果规则以斜线') + ' <code>/</code>' + pt('开头，则从头开始要匹配全部') + '</li>\
                        <li>' + pt('如果规则以') + ' <code>/</code>' + pt('结尾，则要匹配监控路径的末尾') + '</li>\
                        <li><code>?</code> ' + pt('匹配任何字符，但不包括') + '<code>/</code></li>\
                        <li><code>*</code> ' + pt('匹配0或多个字符，但不包括') + '<code>/</code></li>\
                        <li><code>**</code> ' + pt('匹配0或多个字符，可以是') + '<code>/</code></li>\
                    </ul>\
                </div>\
            </div>'
    });
...
function lsyncdSend(){
    api.post('lsyncd_list', '', function(data){
        ...
        con += '<div style="padding-top:1px;">\
                <button class="btn btn-success btn-sm" onclick="createSendTask();">' + pt('创建发送任务') + '</button>\
                <button class="btn btn-success btn-sm" onclick="lsyncdConfLog();">' + pt('自动同步日志') + '</button>\
            </div>';
...
function rsyncdReceive(){
	api.post('rec_list', '', function(data){
        ...
        con += '<div style="padding-top:1px;">\
                <button class="btn btn-success btn-sm" onclick="rsyncdConf();">' + pt('配置') + '</button>\
                <button class="btn btn-success btn-sm" onclick="rsyncdLog();">' + pt('日志') + '</button>\
            </div>';
