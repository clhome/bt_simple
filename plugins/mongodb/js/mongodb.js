...
function mongoDocStatus() {
    ...
    $.post('/plugins/run', {name:'mongodb', func:'run_doc_info'}, function(data) {
		...
		var con = '<div class="divtable">\
						...
						<thead><th>' + pt('库名') + '</th><th>' + pt('大小') + '</th><th>' + pt('存储大小') + '</th><th>' + pt('数据') + '</th><th>' + pt('索引') + '</th><th>' + pt('文档数据') + '</th><th>' + pt('对象') + '</th></thead>\
						<tbody>'+t+'<tbody>\
				</table></div>';
...
function mongoReplStatus() {
    ...
    $.post('/plugins/run', {name:'mongodb', func:'run_repl_info'}, function(data) {
		...
		con += '<div class="divtable" style="margin-top:5px;">\
				<table class="table table-hover table-bordered" style="width: 660px;">\
					<thead><th>IP</th><th>' + pt('状态') + '</th><th>' + pt('在线') + '</th></thead>\
...
function mongoReplCfgReplSetName(){
    ...
    layer.open({
        ...
        success: function(){
    		...
    		// if (dbs.length == 0 ){
    		// 	selectHtml += "<option value=''>' + pt('无') + '</option>";
    		// }
        ...
        yes:function(index){
            ...
            if (data['name'] == ''){
            	layer.msg(pt("副本名称不能为空"));
            	return;
            }
            api.post('repl_set_name', '',data, function(data){
                var rdata = JSON.parse(data.data);
...
function mongoReplCfgNodes(idx,host, priority, votes, arbiterOnly){
	...
	layer.open({
        ...
        yes:function(index){
            ...
            data['idx'] = idx;
            api.post('repl_set_node', '',data, function(data){
                var rdata = JSON.parse(data.data);
                showMsg(rdata.msg,function(){
                	if (rdata['status']){
                		layer.close(index);
...
function mongoReplCfgDelNode(host){
	api.post('del_repl_node', '', {"node":host}, function(data){
		var rdata = JSON.parse(data.data);
...
function mongoReplCfg(){
	layer.open({
        ...
        yes:function(){
        	api.post('repl_init', '', '', function(data){
        		var rdata = JSON.parse(data.data);
				showMsg(rdata.msg,function(){
					mongoReplStatus();
		        },{icon: rdata.status ? 1 : 2});
        ...
        btn5:function(){
        	api.post('repl_close', '', '', function(data){
        		var rdata = JSON.parse(data.data);
				showMsg(rdata.msg,function(){
					if (rdata['status']){
						mongoReplStatus();
...
function mongoSetConfig() {
    api.post('get_config', '','',function(data){
        var rdata = JSON.parse(data.data);
        if (!rdata['status']){
        	layer.msg(rdata['msg']);
        	return;
...
function syncGetDatabase(){
    api.post('sync_get_databases', '', '', function(data){
        var rdata = JSON.parse(data.data);
        showMsg(rdata.msg,function(){
            dbList();
        },{ icon: rdata.status ? 1 : 2 });
            ...
            success:function(layer_id, layer_index){
		        ...
		        for (var i = 0; i < rdata.collection_list.length; i++) {
		            tbody += '<tr>\
		                    ...
		                    <td>' + '<a href="javascript:;" data-index="'+i+'" class="delete btlink" title="删除">' + pt('删除') + '</a>'+'</td>\
		                </tr>';
		        }
...
function mgdbReadme(){
    ...
    readme += '<li>' + pt('认证同步说明') + '</li>';
    readme += '<li>' + pt('root/用户,配置Key完全一致才能同步。') + '</li>';
    readme += '</ul>';
