...
def submitRedisConf():
    ...
    for g in gets:
        if g in args:
            ...
            if g == 'requirepass' and val_str == '':
                content = re.sub(r'^(#)?requirepass\s*([.0-9A-Za-z_& ~]*)', '#requirepass ""', content, flags=re.M)
            elif g == 'requirepass' and val_str != '':
                content = re.sub(r'^#requirepass\s*([.0-9A-Za-z_& ~]*)', 'requirepass ' + val_str, content, flags=re.M)
                rep_r = r'^(requirepass)\s*([.0-9A-Za-z_& ~]*)'
                content = re.sub(rep_r, 'requirepass ' + val_str, content, flags=re.M)

            elif g == 'masterauth' and val_str == '':
                content = re.sub(r'^(#)?masterauth\s*([.0-9A-Za-z_& ~]*)', '#masterauth ""', content, flags=re.M)
            elif g == 'masterauth' and val_str != '':
                content = re.sub(r'^#masterauth\s*([.0-9A-Za-z_& ~]*)', 'masterauth ' + val_str, content, flags=re.M)
                rep_m = r'^(masterauth)\s*([.0-9A-Za-z_& ~]*)'
                content = re.sub(rep_m, 'masterauth ' + val_str, content, flags=re.M)
