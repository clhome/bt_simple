...
def openPort():
    try:
        ...
        YfFirewall.instance().addAcceptPort('3000', 'grafana', 'port')
        return port
...
def gOp(method):
    ...
    yf.execShell('systemctl ' + method + ' '+getPluginName())
    if data[1] == '':
        return 'ok'
    return data[1]
...
def initdInstall():
    ...
    data = yf.execShell('systemctl enable grafana')
    if data[1] != '':
        return data[1]
    return 'ok'
...
def initdUinstall():
    ...
    data = yf.execShell('systemctl disable grafana')
    if data[1] != '':
        return data[1]
    return 'ok'
