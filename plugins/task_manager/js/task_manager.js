...
function get_process_list(sortx, reverse, rx) {
    ...
    if (!rx) {
      var loadT = layer.msg(pt('正在获取进程列表..'), {icon: 16, time: 0, shade: [0.3, '#000']})
    }
    ...
    tmPostCallback('get_process_list', {sortx: sortx,reverse: reverse,search:search_val}, function(rdata){
        ...
        var tbody = '<thead title="' + pt('右键可设置表头') + '" id="change_thead">\
					<tr style="cursor: pointer;">\
						<th style="width:160px;' + (data.meter_head.ps ? '' : 'display:none;') + '" class="pro_name pro_ps"  onclick="get_process_list(\'ps\',\'' + res_list[reverse] + '\')">' + pt('应用名称') + '</th>\
						<th class="pro_pid" style="' + (data.meter_head.pid ? '' : 'display:none;') + '" onclick="get_process_list(\'pid\',\'' + res_list[reverse] + '\')">' + pt('PID') + '</th>\
						<th class="pro_threads" style="' + (data.meter_head.threads ? '' : 'display:none;') + '" onclick="get_process_list(\'threads\',\'True\')">' + pt('线程') + '</th>\
						<th style="width:80px;' + (data.meter_head.user ? '' : 'display:none;') + '" class="pro_user" onclick="get_process_list(\'user\',\'' + res_list[reverse] + '\')">' + pt('用户') + '</th>\
						<th style="' + (data.meter_head.cpu_percent ? '' : 'display:none;') + '" class="pro_cpu_percent" onclick="get_process_list(\'cpu_percent\',\'True\')">' + pt('CPU') + '</th>\
						<th class="pro_memory_used" style="' + (data.meter_head.memory_used ? '' : 'display:none;') + '" onclick="get_process_list(\'memory_used\',\'True\')">' + pt('内存') + '</th>\
						<th style="width:70px;' + (data.meter_head.io_read_bytes ? '' : 'display:none;') + '" class="pro_io_read_speed" onclick="get_process_list(\'io_read_speed\',\'True\')">' + pt('io读') + '</th>\
						<th style="width:70px;' + (data.meter_head.io_write_bytes ? '' : 'display:none;') + '" class="pro_io_write_speed" onclick="get_process_list(\'io_write_speed\',\'True\')">' + pt('io写') + '</th>\
						<th style="width:70px;' + (data.meter_head.up ? '' : 'display:none;') + '" class="pro_up" onclick="get_process_list(\'up\',\'True\')">' + pt('上行') + '</th>\
						<th style="width:70px;' + (data.meter_head.down ? '' : 'display:none;') + '" class="pro_down" onclick="get_process_list(\'down\',\'True\')">' + pt('下行') + '</th>\
						<th class="pro_connects" style="' + (data.meter_head.connects ? '' : 'display:none;') + '" onclick="get_process_list(\'connects\',\'True\')">' + pt('连接') + '</th>\
						<th class="pro_status" style="' + (data.meter_head.status ? '' : 'display:none;') + '" onclick="get_process_list(\'status\',\'' + res_list[reverse] + '\')">' + pt('状态') + '</th>\
						<th style="cursor:text;">' + pt('操作') + '</th>\
					</tr>\
				</thead>\
				<tbody>' + tbody_tr + '</tbody>';
        $("#TaskManagement").html(tbody);
        var topMsg = '<div class="mini-info-box">\
				<div class="mini-info-con"><p><span class="tname">' + pt('CPU：') + '</span>' + data.info.cpu + '%</p><p><span class="tname">' + pt('内存：') + '</span>' + toSize(data.info.mem) + '</p></div>\
				...
				<div class="mini-info-con"><p><span class="tname">' + pt('进程数：') + '</span>' + data.process_list.length + '</p><p><span class="tname">' + pt('磁盘：') + '</span>' + toSize(data.info.disk) + '</p></div>\
			</div>';
        ...
        if(getCookie('table_config_tip')=='false'||!getCookie('table_config_tip')){
            layer.tips(pt('点击可设置表头'), '.setting_btn', {
                tips: [1, '#20a53a'],
                time: 3000
            });
...
function createProcessTable(getboday, data) {
    ...
    for (var i = 0; i < realProcess.length; i++) {
        ...
        if ('io_read_bytes' in realProcess[i]){
            ...
            tbody_td += '<td style="' + (data?(data.meter_head.up ? '' : 'display:none;'):'') + '" title="' + msgTpl(pt('上行速度：{1}/秒'),[toSize(realProcess[i].up)]) + '\n' + msgTpl(pt('发包速度：{1}个/秒'),[realProcess[i].up_package]) + '">' + toSize(realProcess[i].up).replace(' ', '') + '</td>';
            tbody_td += '<td style="' + (data?(data.meter_head.down ? '' : 'display:none;'):'') + '" title="' + msgTpl(pt('下行速度：{1}/秒'),[toSize(realProcess[i].down)]) + '\n' + msgTpl(pt('收包速度：{1}个/秒'),[realProcess[i].down_package]) + '">' + toSize(realProcess[i].down).replace(' ', '') + '</td>';
        }
        ...
        tbody_tr += '<tr ' + selected + selected_one + ' onclick="click_process_tr(event,' + realProcess[i].pid + ',' + realProcess[i].fpid + ')" >\
			...
			<td><a class="btlink" onclick="' + kill_process + '(' + realProcess[i].pid + ',' + realProcess[i].fpid + ')">' + pt('结束') + '</a></td>\
		</tr>';
    }
...
function get_resource_list() {
    ...
    $.post(url, function (data) {
      ...
      var tbody = '<thead>\
        	<tr style="cursor: pointer;">\
        		<th style="width:120px;" class="pro_name pro_ps" onclick="get_process_list(\'ps\',\'' + res_list[reverse] + '\')">' + pt('应用名称') + '</th>\
        		<th class="pro_pid" onclick="get_process_list(\'pid\',\'' + res_list[reverse] + '\')">' + pt('PID') + '</th>\
        		<th style="width:50px;"class="pro_threads" onclick="get_process_list(\'threads\',\'True\')">' + pt('线程') + '</th>\
        		<th style="width:60px;" class="pro_user" onclick="get_process_list(\'user\',\'' + res_list[reverse] + '\')">' + pt('用户') + '</th>\
        		<th style="width:70px;" class="pro_cpu_percent" onclick="get_process_list(\'cpu_percent\',\'True\')">' + pt('CPU') + '</th>\
        		<th style="width:70px;" class="pro_memory_used" onclick="get_process_list(\'memory_used\',\'True\')">' + pt('内存') + '</th>\
        		<th style="width:70px;" class="pro_io_read_speed" onclick="get_process_list(\'io_read_speed\',\'True\')">' + pt('io读') + '</th>\
        		<th style="width:70px;" class="pro_io_write_speed" onclick="get_process_list(\'io_write_speed\',\'True\')">' + pt('io写') + '</th>\
        		<th style="width:70px;" class="pro_up" onclick="get_process_list(\'up\',\'True\')">' + pt('上行') + '</th>\
        		<th style="width:70px;" class="pro_down" onclick="get_process_list(\'down\',\'True\')">' + pt('下行') + '</th>\
        		<th style="width:50px;" class="pro_connects" onclick="get_process_list(\'connects\',\'True\')">' + pt('连接') + '</th>\
        		<th style="width:50px;" class="pro_status" onclick="get_process_list(\'status\',\'' + res_list[reverse] + '\')">' + pt('状态') + '</th>\
        		<th style="width:50px;cursor:text">' + pt('操作') + '</th>\
        	</tr>\
        </thead>\
        <tbody>' + tbody_tr + '</tbody>';
...
function get_cron_list() {
    var loadT = layer.msg(pt('获取计划任务列表..'), {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_cron_list', {search:search_val}, function(rdata){
        ...
        var tbody = '<thead>\
					<tr>\
						<th width="150">' + pt('周期') + '</th>\
						<th >' + pt('执行') + '</th>\
						<th overflow="hidden" width="40%">' + pt('描述') + '</th>\
						<th width="50">' + pt('操作') + '</th>\
					</tr>\
				</thead>\
				<tbody>' + tbody_tr + '</tbody>';
        $("#TaskManagement").html(tbody);
...
function get_network_list(rflush) {
    var loadT = layer.msg(pt('正在获取网络状态..'), {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_network_list', {search:search_val}, function(rdata){
        layer.close(loadT);
        if (rdata.data['is_mac']){
            tbody_tr += "<tr><td colspan='6' style='text-align:center;'>" + pt('mac无法使用') + "</td></tr>";
            tbody = "<thead>\
                <tr>\
                    <th>" + pt('协议') + "</th>\
                    <th>" + pt('本地地址') + "</th>\
        ...
        var topMsg = '<div class="mini-info-box" style="width:666px">\
				<div class="mini-info-con" style="width:25%">\
					<p><span class="tname">' + pt('总发送：') + '</span>' + toSize(rdata.state.upTotal) + '</p>\
					<p><span class="tname">' + pt('总接收：') + '</span>' + toSize(rdata.state.downTotal) + '</p>\
				</div>\
				<div class="mini-info-con" style="width:25%">\
					<p><span class="tname">' + pt('上行：') + '</span>' + toSize(rdata.state.up) + '</p>\
					<p><span class="tname">' + pt('下行：') + '</span>' + toSize(rdata.state.down) + '</p>\
				</div>\
				<div class="mini-info-con" style="width:25%;border-right:#DBDBEA 1px solid">\
					<p><span class="tname">' + pt('总发包：') + '</span>' + to_max(rdata.state.upPackets) + '</p>\
					<p><span class="tname">' + pt('总收包：') + '</span>' + to_max(rdata.state.downPackets) + '</p>\
				</div>\
				<div class="mini-info-con" style="width:25%;">\
					<p><span class="tname">' + pt('包发送/秒：') + '</span>' + to_max(rdata.state.upPackets_s) + '</p>\
					<p><span class="tname">' + pt('包接收/秒：') + '</span>' + to_max(rdata.state.downPackets_s) + '</p>\
				</div>\
			</div>';
        $("#load_average").html(topMsg).show();
        $(".table-cont").css("height", "500px");
...
function to_max(num) {
    if (num > 10000) {
      ...
      if (num > 10000) {
        num = num / 10000;
        return num.toFixed(5) + ' ' + pt('亿');
      }
      return num.toFixed(5) + ' ' + pt('万');
    }
    return num;
}
...
function get_who_list() {
    var loadT = layer.msg(pt('正在获取用户会话列表..'), {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_who', {search:search_val}, function(data){
        ...
        var tbody = '<thead>\
					<tr>\
						<th width="130">' + pt('用户') + '</th>\
						<th width="130">' + pt('PTS') + '</th>\
						<th>' + pt('登陆IP') + '</th>\
						<th>' + pt('登陆时间') + '</th>\
						<th width="80">' + pt('操作') + '</th>\
					</tr>\
				</thead>\
				<tbody>' + tbody_tr + '</tbody>';
        $("#TaskManagement").html(tbody);
...
function get_run_list() {
    var loadT = layer.msg(pt('正在获取启动项列表..'), {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_run_list', {search:search_val}, function(rdata){
        ...
        if (rdata.data['is_mac']){
            ...
            var tbody = '<thead>\
                    <tr>\
                        <th width="170">' + pt('名称') + '</th>\
                        <th width="200">' + pt('启动路径') + '</th>\
                        <th width="100">' + pt('文件大小') + '</th>\
                        <th width="100">' + pt('文件权限') + '</th>\
                        <th>' + pt('描述') + '</th>\
                        <th width="50">' + pt('操作') + '</th>\
                    </tr>\
                </thead>\
        ...
        var tbody = '<thead>\
				...
				<tbody>' + tbody_tr + '</tbody>';
        $("#TaskManagement").html(tbody);
        var topMsg = '<div class="mini-level">' + pt('当前运行级别：') + ' level-' + rdata.run_level + '</div>';
        $("#load_average").html(topMsg).show();
        $(".table-cont").css("height", "500px");
        show_task();
    });
...
function get_service_list() {
    var loadT = layer.msg(pt('正在获取服务列表..'), {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_service_list', {search:search_val}, function(rdata){
        layer.close(loadT);
        if (rdata.data['is_mac']){
            tbody_tr += "<tr><td colspan='10' style='text-align:center;'>" + pt('mac无法使用') + "</td></tr>";
            var tbody = '<thead>\
                    <tr>\
                        <th>' + pt('名称') + '</th>\
                        ...
                        <th style="overflow:hidden">' + pt('描述') + '</th>\
                        <th width="50">' + pt('操作') + '</th>\
                    </tr>\
                </thead>\
                <tbody>' + tbody_tr + '</tbody>';
        ...
        var tbody = '<thead>\
				...
				<tbody>' + tbody_tr + '</tbody>';
        $("#TaskManagement").html(tbody);
        var topMsg = '<div class="mini-level">' + pt('当前运行级别：') + ' level-' + rdata.runlevel + '</div>';
        $("#load_average").html(topMsg).show();
        $(".table-cont").css("height", "500px");
        show_task();
    });
...
function get_user_list() {
    var loadT = layer.msg(pt('正在获取用户列表..'), {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_user_list', {search:search_val}, function(data){
        ...
        var tbody = '<thead>\
			<tr>\
				<th width="140">' + pt('用户名') + '</th>\
				<th width="140">' + pt('Home目录') + '</th>\
				<th width="140">' + pt('用户组') + '</th>\
				<th width="50">UID</th>\
				<th width="50">GID</th>\
				<th width="150">' + pt('登陆Shell') + '</th>\
				<th style="overflow:hidden">' + pt('描述') + '</th>\
				<th width="50">' + pt('操作') + '</th>\
			</tr>\
		</thead>\
...
function userdel(user) {
    safeMessage(pt('删除用户') + '【' + user + '】', pt('删除后可能导致您的环境无法正常运行,继续吗？'), function () {
        var loadT = layer.msg(pt('正在删除用户..') + ' [' + user + ']', {icon: 16, time: 0, shade: [0.3, '#000']});
...
function kill_process(pid, fpid) {
    ...
    var w = layer.confirm(pt('您是否要结束进程？') + ' (' + pid + ')', {
        btn: [pt('结束'), pt('取消')], //按钮
        title: pt('结束进程') + ' ' + pid,
        closeBtn: 2
    }, function () {
        var loadT = layer.msg(pt('正在结束进程..') + ' [' + pid + ']', {icon: 16, time: 0, shade: [0.3, '#000']});
...
function kill_process_all(pid) {
    var w = layer.confirm(pt('您是否要结束进程？') + ' (' + pid + ')', {
        ...
        var loadT = layer.msg(pt('正在结束父进程..') + ' [' + pid + ']', {icon: 16, time: 0, shade: [0.3, '#000']});
...
function remove_service(serviceName) {
    safeMessage(pt('删除服务') + '【' + serviceName + '】', pt('删除后可能导致您的环境无法正常运行,继续吗？'), function () {
        var loadT = layer.msg(pt('正在删除服务..') + ' [' + serviceName + ']', {icon: 16, time: 0, shade: [0.3, '#000']});
...
function remove_cron(index) {
    safeMessage(pt('删除计划任务') + '[' + index + ']', pt('删除后将无法恢复,继续吗？'), function () {
        var loadT = layer.msg(pt('正在删除计划任务..'), {icon: 16, time: 0, shade: [0.3, '#000']});
...
function pkill_session(pts) {
    safeMessage(pt('强制断开会话') + '[' + pts + ']', pt('强制断开此会话吗？'), function () {
        var loadT = layer.msg(pt('正在断开会话..'), {icon: 16, time: 0, shade: [0.3, '#000']});
...
function set_runlevel_state(runlevel, serviceName) {
    var loadT = layer.msg(pt('正在设置服务..') + ' [' + serviceName + ']', {icon: 16, time: 0, shade: [0.3, '#000']});
...
function get_process_info(pid) {
    var loadT = layer.msg(pt('正在获取进程信息..') + ' [' + pid + ']', {icon: 16, time: 0, shade: [0.3, '#000']});
    tmPostCallback('get_process_info', {pid:pid}, function(data){
				...
				<table class="table">\
					<tbody>\
						...
						<tr>\
							...
							<th>' + pt('用户') + '</th><td>' + rdata.user + '</td>\
							<th>' + pt('线程') + '</th><td>' + rdata.threads + '</td>\
						</tr>\
						<tr>\
							<th>Socket</th><td>' + rdata.connects + '</td>\
							<th>' + pt('IO读') + '</th><td>' + toSize(rdata.io_read_bytes) + '</td>\
							<th>' + pt('IO写') + '</th><td>' + toSize(rdata.io_write_bytes) + '</td>\
						</tr>\
						<tr>\
		...
		<div class="mini-info">\
			<button class="btn btn-sm btn-default mr5" onclick="open_path(\'' + rdata.exe + '\')">' + pt('打开文件位置') + '</button>\
			<button class="btn btn-sm btn-default" onclick="kill_process_all(' + rdata.pid + ')">' + pt('结束进程树') + '</button>\
		</div>';

        TaskProcessLayerIndex = layer.open({
            type: 1,
            title: pt('进程属性') + ' [' + rdata.name + '] -- ' + rdata.exe,
            area: '750px',
            closeBtn: 2,
            shadeClose: false,
            content: cbody
...
function dropAddress(address) {
    layer.confirm(pt('您真的要屏蔽此IP吗？'), {icon: 3, closeBtn: 2}, function () {
        var loadT = layer.msg(pt('正在屏蔽指定IP..'), {icon: 16, time: 0, shade: [0.3, '#000']});
