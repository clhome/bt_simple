...
class mainClass(object):
    ...
    def get_process_list_mac(self, args = {}):
        ...
        self.old_info['cpu_time'] = self.new_info['cpu_time']
        self.old_info['time'] = self.new_info['time']
        return data
            ...
            try:
                ...
                with p.oneshot():
                    try:
                        ...
                        try:
                            p_mem_basic = p.memory_info()
                            uss_mem = p_mem_basic.rss
                            rss_mem = p_mem_basic.rss
                        except Exception:
                            continue

                    if rss_mem == 0: continue
                    pio = p.io_counters()
                    p_cpus = p.cpu_times()
                    p_state = p.status()
                    if p_state == 'running': info['activity'] += 1
        ...
        try:
            ...
            if self.is_mac:
                ...
                try:
                    ...
                    processInfo['io_write_bytes'] = 0
                    processInfo['io_read_bytes'] = 0
            # p_cpus= p.cpu_times()
