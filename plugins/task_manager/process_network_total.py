...
class process_network_total:
    ...
    def handle_packet(self, pcap_data):
        if len(pcap_data) < 34: return
        eth_type = struct.unpack('!H', pcap_data[12:14])[0]
        if eth_type == 0x0800: # IPv4
            ...
            if len(pcap_data) < 62: return
            src_ip = pcap_data[22:38]
            dst_ip = pcap_data[38:54]
            src_port = pcap_data[54:56]
            dst_port = pcap_data[56:58]
    ...
    def hex_to_ip6_bin(self, hex_ip):
        ip_bin = b''
        for i in range(0, 32, 8):
            part = hex_ip[i:i+8]
            val = int(part, 16)
            ip_bin += struct.pack('<I', val)
        return ip_bin
    ...
    def get_tcp_stat(self,force = False):
        ...
        tcp_stat_files = [('/proc/net/tcp', False), ('/proc/net/tcp6', True)]
        if force: self.get_process_inodes(force)
        
        for tcp_stat_file, is_ipv6 in tcp_stat_files:
            if not os.path.exists(tcp_stat_file): continue
            try:
                ...
                for i in tcp_stat_list:
                    ...
                    if is_ipv6:
                        local_ip_bin = self.hex_to_ip6_bin(tcp_tmp[1].split(':')[0])
                        if local_ip_bin == b'\x00'*15 + b'\x01': continue
                        
                        remote_ip_bin = self.hex_to_ip6_bin(tcp_tmp[2].split(':')[0])
                        if local_ip_bin == remote_ip_bin: continue
                        if remote_ip_bin == b'\x00'*16: continue
                        
                        pid = self.inode_to_pid(inode, force)
                        if not pid: continue
                        
                        local_port = int(tcp_tmp[1].split(':')[1], 16)
                        key = local_ip_bin + b':' + struct.pack('!H', local_port)
