...
function swapStatus() {
    api.post('swap_status', '', {}, function(data){
        ...
        spCon += '    <h4 style="margin: 0 0 12px 0; font-size: 14px; font-weight: bold; color: #333; border-left: 3px solid #20a53a; padding-left: 8px;">' + pt('📊 系统当前 Swap 状态') + '</h4>';
        ...
        spCon += '        <div style="font-size: 12px; color: #666; margin-bottom: 4px;">' + pt('系统实际总 Swap (已启用)') + '</div>';
        ...
        // 状态卡片 2：插件专属 Swap
        spCon += '      <div style="flex: 1; padding: 12px; background: #fff; border: 1px solid #e1e4e8; border-radius: 4px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">';
        spCon += '        <div style="font-size: 12px; color: #666; margin-bottom: 4px;">' + pt('插件专属 Swapfile 文件') + '</div>';
        ...
        spCon += '    <h4 style="margin: 0 0 15px 0; font-size: 14px; font-weight: bold; color: #333; border-left: 3px solid #007bff; padding-left: 8px;">' + pt('⚙️ 调整插件虚拟内存配置') + '</h4>';
        ...
        spCon += '        <span style="font-size: 13px; color: #555; font-weight: 500;">' + pt('配置容量选择:') + '</span>';
        ...
        selectHtml += '  <option value="">' + pt('请选择...') + '</option>';
        ...
        // 提交修改按钮
        spCon += '      <div>';
        spCon += '        <button class="btn btn-success btn-sm" style="padding: 6px 20px; font-size: 13px; font-weight: bold; border-radius: 4px;" onclick="submitSwap()">' + pt('提交修改') + '</button>';
        spCon += '      </div>';
...
function submitSwap(){
    ...
    if (!size) {
        layer.msg(pt("请先选择您想要配置的 Swap 容量！"), { icon: 0 });
        return;
    }
...
function readme(){
    ...
    readme += '<h4 style="margin-top:0; color:var(--title-color, #222); font-weight:bold; border-bottom:1px solid var(--border-color, #eee); padding-bottom:8px;">' + pt('Swap (虚拟内存) 配置与使用指南') + '</h4>';
    readme += '<p style="margin-top:10px; text-indent:2em;"><b>' + pt('什么是 Swap？') + '</b>' + pt('Swap 也就是虚拟内存，当物理内存 (RAM) 不足时，Linux 系统会自动将部分闲置内存数据临时交换到磁盘上的专属 Swap 空间中，以保证系统稳定运行。配置 Swap能极大防止因内存溢出 (OOM) 导致关键服务 (如 MySQL、Nginx 等) 被系统强制杀死的现象。') + '</p>';
    readme += '<h5 style="margin-top:20px; color:var(--title-color, #333); font-weight:bold;">' + pt('1. 不同情况下的 Swap 大小配置建议：') + '</h5>';
    ...
    readme += '      <th style="padding:10px; border:1px solid var(--border-color, #ddd); font-weight:bold; color:var(--title-color, #444);">' + pt('物理内存 (RAM)') + '</th>';
    readme += '      <th style="padding:10px; border:1px solid var(--border-color, #ddd); font-weight:bold; color:var(--title-color, #444);">' + pt('推荐 Swap 设定规则') + '</th>';
    readme += '      <th style="padding:10px; border:1px solid var(--border-color, #ddd); font-weight:bold; color:var(--title-color, #444);">' + pt('最佳建议值 (MB)') + '</th>';
    readme += '    </tr>';
    readme += '  </thead>';
    readme += '  <tbody>';
    readme += '    <tr><td style="padding:10px; border:1px solid var(--border-color, #ddd);"><b>' + pt('小于 2GB') + '</b></td><td style="padding:10px; border:1px solid var(--border-color, #ddd);">' + pt('物理内存的') + ' <b>' + pt('2倍') + '</b></td><td style="padding:10px; border:1px solid var(--border-color, #ddd); color:#28a745; font-weight:bold;">1024 MB - 2048 MB</td></tr>';
    readme += '    <tr><td style="padding:10px; border:1px solid var(--border-color, #ddd);"><b>2GB - 8GB</b></td><td style="padding:10px; border:1px solid var(--border-color, #ddd);">' + pt('与物理内存') + '<b>' + pt('等大') + '</b>' + pt('或') + ' <b>' + pt('1.5倍') + '</b></td><td style="padding:10px; border:1px solid var(--border-color, #ddd); color:#28a745; font-weight:bold;">2048 MB - 4096 MB</td></tr>';
    readme += '    <tr><td style="padding:10px; border:1px solid var(--border-color, #ddd);"><b>8GB - 16GB</b></td><td style="padding:10px; border:1px solid var(--border-color, #ddd);">' + pt('配置约') + ' <b>4GB</b> ' + pt('即可') + '</td><td style="padding:10px; border:1px solid var(--border-color, #ddd); color:#28a745; font-weight:bold;">4096 MB</td></tr>';
    readme += '    <tr><td style="padding:10px; border:1px solid var(--border-color, #ddd);"><b>' + pt('大于 16GB') + '</b></td><td style="padding:10px; border:1px solid var(--border-color, #ddd);">' + pt('配置') + ' <b>4GB - 8GB</b> ' + pt('即可') + '</td><td style="padding:10px; border:1px solid var(--border-color, #ddd); color:#28a745; font-weight:bold;">4096 MB - 8192 MB</td></tr>';
    readme += '  </tbody>';
    readme += '</table>';
    readme += '<h5 style="margin-top:20px; color:var(--title-color, #333); font-weight:bold;">' + pt('2. 使用注意事项与性能提醒：') + '</h5>';
    readme += '<ul style="padding-left:20px; margin-top:5px; list-style-type:disc;">';
    readme += '  <li style="margin-bottom:8px;"><b>' + pt('磁盘性能关联') + '</b>' + pt('：Swap 位于磁盘上，读写性能远低于真实的物理内存。因此，建议将 Swap 空间建立在') + ' <b>' + pt('SSD 固态硬盘') + '</b> ' + pt('上。如果在传统的 HDD 机械硬盘上频繁使用 Swap，可能会使磁盘 I/O 瞬间飙高，从而导致系统整体响应变慢。') + '</li>';
    readme += '  <li style="margin-bottom:8px;"><b>' + pt('后台创建耗时') + '</b>' + pt('：当您在“配置调整”中手工修改提交时，后台需要通过磁盘写入 (') + '<code>dd</code> ' + pt('命令) 重新生成指定大小的文件。配置的 Swap 容量越大 (如 4GB 以上)，因需要高负载写入，创建时间可能会持续') + '<b>' + pt('几秒到几十秒') + '</b>' + pt('，这属于正常现象，请在提交后耐心等待提示成功。') + '</li>';
    readme += '  <li style="margin-bottom:8px;"><b>' + pt('系统级 Swap 叠加') + '</b>' + pt('：本插件创建的 Swapfile 属于插件专属虚拟内存，挂载启用后，会自动与您系统本身已有的 Swap 分区或文件') + '<b>' + pt('叠加合并生效') + '</b>' + pt('，两者不产生任何冲突。') + '</li>';
    readme += '</ul>';
    readme += '</div>';
