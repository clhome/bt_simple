...
function readme(){
    ...
    readme += '<p style="margin-top:10px; text-indent:2em;"><b>' + pt('什么是 Swap？') + '</b>Swap 也就是虚拟内存，当物理内存 (RAM) 不足时，Linux 系统会自动将部分闲置内存数据临时交换到磁盘上的专属 Swap 空间中，以保证系统稳定运行。配置 Swap能极大防止因内存溢出 (OOM) 导致关键服务 (如 MySQL、Nginx 等) 被系统强制杀死的现象。</p>';
    ...
    readme += '  <li style="margin-bottom:8px;"><b>' + pt('后台创建耗时') + '</b>：当您在“配置调整”中手工修改提交时，后台需要通过磁盘写入 (<code>dd</code> 命令) 重新生成指定大小的文件。配置的 Swap 容量越大 (如 4GB 以上)，因需要高负载写入，创建时间可能会持续<b>' + pt('几秒到几十秒') + '</b>' + pt('，这属于正常现象，请在提交后耐心等待提示成功。') + '</li>';
