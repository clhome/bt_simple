...
20260625 14:15 网站列表修改PHP版本后自动更新

- [/] 网站列表修改PHP版本后自动更新
  - [x] 1. 编写并提交 implementation_plan.md 提交用户审核
  - [/] 2. 修改 `web/static/app/site.js` 的 `setPHPVersion` 方法，在成功保存 PHP 版本后进行局部 DOM 更新
...
- [x] 站点 SSL 配置页增加手动续签按钮
  ...
  - [x] 4. 进行手动验证，测试直达 SSL 证书页面后点击“手动续签”按钮是否能正常拉起进度弹窗、展示滚动日志并续签成功
  - [x] 5. 编写并提供 walkthrough.md 报告

---

20260625 15:00 停止的站点行增加浅红色底纹

- [x] 停止的站点行增加浅红色底纹
  - [x] 1. 在 `web/templates/default/site.html` 中添加 `.danger-row` 浅红色底纹及悬停样式
  - [x] 2. 在 `web/static/app/site.js` 的 `getWeb` 方法中，为已停止的站点行（`tr`）动态添加 `danger-row` 类名
...
20260626 17:15 修复手动续签 SSL 时目录变更后，自动续签失效、假成功 Bug
已修复。这次改为用纯 Python（mw.readFile + re.sub + mw.writeFile）来修改 acme.sh 的配置文件，不再依赖 sed。
