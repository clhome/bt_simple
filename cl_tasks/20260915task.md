...
- [x] 440. 补全 MySQL 与 MariaDB 6 国语言包（`zh-CN`, `zh-TW`, `en`, `de`, `fr`, `it`）：录入 10 个数据权限相关词条（包含下拉选项、红字徽章提示 title、弹窗动态释义）。
- [x] 441. 修复 `plugins/mysql/js/mysql.js` 与 `plugins/mariadb/js/mariadb.js` 数据表格折行问题：使用 Flexbox 结合 `white-space: nowrap` 彻底解决权限图标与红字标识掉行，优化英文表头防折行。
- [x] 442. 编写专项自动化测试套件（`test/test_mysql_mariadb_rw_i18n_and_layout.py`）并回归全量测试，确保 6 国语言 100% 覆盖且布局稳定。
- [x] 443. 修复前端 `plugins/mysql/js/mysql.js` 与 `plugins/mariadb/js/mariadb.js` 中 `openPhpmyadmin` 跳转逻辑：彻底废除 `url.indexOf('phpmyadmin') == -1` 错误校验，安全注入参数并跳转。
- [x] 444. 修复后端 `plugins/mysql/index.py` 与 `plugins/mariadb/index.py` 中 `repairTable`、`optTable`、`alterTable`：消除写死返回 False 严重缺陷，准确返回执行结果与错误捕获。
- [x] 445. 优化 MySQL/MariaDB 工具箱前端交互与多语言：增加一键全库优化、针对 InnoDB/MyISAM 引擎的智能识别反馈，并补齐 6 国语言词条。
- [ ] 446. 编写专项自动化测试套件（`test/test_mysql_mariadb_tools_and_pma.py`）并执行全量回归，更新 `task.md` 与 `walkthrough.md`。
